...
class Korwil1_model2 extends CI_Model
{
    ...
    function json($data) {
        ...
        $this->datatables2->select("caseids,json_extract(uncompress(questionnaire), '$.id.P101')as kode_p,json_extract(uncompress(questionnaire), '$.BLOK_13.PROP_TEXT')as prov,count(json_extract(uncompress(questionnaire), '$.id.P101'))as jmh_ruta");
        $this->datatables2->from('ssgi2022_dict2');
        //$this->datatables->where("json_extract(uncompress(questionnaire), '$.id.P101')=13");
        $this->datatables2->add_column("jmhbsbps",'$1','jmhbstprovk2(caseids)');
        ...
        $this->datatables2->where_in("json_extract(uncompress(`questionnaire`), '$.BLOK_13.NM_KORWIL')",$data);
        $this->datatables2->group_by("json_extract(uncompress(questionnaire), '$.id.P101')");
        //add this line for join
        //$this->datatables->join('table2', 'vis.field = table2.field');
        $this->datatables2->add_column('action', anchor(site_url('korwil2/provinsi/$1'),'Lihat Daftar Kab/Kota', array('class' => 'btn btn-default btn-sm')), 'substr(caseids,0,2)');
